...
public class Tds_Grid : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        ...
        {
            Gizmos.DrawLine(new Vector3(-10000000, Mathf.Floor(y/this.height)*this.height, 0.0f), new Vector3(10000000, Mathf.Floor(y/this.height)*this.height, 0.0f));
        }

        for (float x = pos.x - 1200.0f; x < pos.x + 1200.0f; x+= this.width)
        {
            Gizmos.DrawLine(new Vector3(Mathf.Floor(x/this.width)*this.width, -10000000, 0.0f), new Vector3(Mathf.Floor(x/this.width)*this.width, 10000000, 0.0f));
        }
    }
}
